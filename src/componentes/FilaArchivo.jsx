...
export const FilaArchivo = ({ archivo, detalleArchivo, manejarCambioArchivo, resetearArchivo }) => {
    ...
    return (
        <div className={`relative bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 transition-all duration-300 hover:shadow-md hover:border-empresa/30 grid grid-cols-1 xl:grid-cols-12 gap-5 items-center ${estaVacio ? 'opacity-60' : ''}`}>
            ...
            <div className="xl:col-span-5 grid grid-cols-12 gap-3">
            ...
            <div className="xl:col-span-3 flex gap-2 h-12">
                <label className={`flex-1 flex items-center justify-center px-1 rounded-2xl border transition-all ${!permiteDobleFaz ? 'opacity-40 cursor-not-allowed bg-gray-100 dark:bg-gray-800' : archivo.esDobleFaz ? 'bg-empresa/10 border-empresa text-empresa cursor-pointer' : 'bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-500 cursor-pointer'}`}>
                    <input type="checkbox" className="hidden" disabled={!permiteDobleFaz} checked={archivo.esDobleFaz} onChange={(evento) => manejarCambioArchivo(archivo.id, 'esDobleFaz', evento.target.checked)} />
                ...
                <label className={`flex-1 flex items-center justify-center px-1 rounded-2xl border transition-all ${!permiteAnillado ? 'opacity-40 cursor-not-allowed bg-gray-100 dark:bg-gray-800' : archivo.anillado ? 'bg-empresa/10 border-empresa text-empresa cursor-pointer' : 'bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-500 cursor-pointer'}`}>
                    <input type="checkbox" className="hidden" disabled={!permiteAnillado} checked={archivo.anillado} onChange={(evento) => manejarCambioArchivo(archivo.id, 'anillado', evento.target.checked)} />
                        ...
                        <div className="space-y-1 border-b dark:border-gray-700 pb-2 mb-2">
                            {parseInt(archivo.paginas) > 0 && (
                                <div className="flex justify-between text-[11px] text-gray-500 dark:text-gray-400">
                                    <span>Impresión ({detalleArchivo.totalPaginas} x ${detalleArchivo.precioUnitarioMayorista})</span>
                                    <span className="dark:text-gray-200 font-bold">${detalleArchivo.subtotalImpresion.toLocaleString('es-AR')}</span>
                                </div>
                            )}
                            {archivo.anillado && (
                                <div className="flex justify-between text-[11px] text-empresa font-black mt-1">
                                    <span>Anillado {detalleArchivo.copias > 1 ? `(${detalleArchivo.copias} u.)` : ''}</span>
                                    <span>+ ${detalleArchivo.costoAnillado.toLocaleString('es-AR')}</span>
                                </div>
                            )}
                            
                            {detalleArchivo.copias > 1 && (
                                <div className="flex justify-between text-[10px] text-blue-500 dark:text-blue-400 font-bold mt-2 pt-2 border-t border-dashed border-gray-200 dark:border-gray-700">
                                    <span>Costo c/juego (con desc.)</span>
                                    <span>${detalleArchivo.totalMayoristaUnJuego.toLocaleString('es-AR')}</span>
                                </div>
                            )}
                        </div>
                        <div className="flex justify-between items-center">
                            <div className="text-left flex flex-col">
                                {detalleArchivo.totalMinorista !== detalleArchivo.totalMayorista && (
                                    <>
                                        <span className="text-[9px] font-bold text-gray-400 uppercase mb-0.5">Lista</span>
