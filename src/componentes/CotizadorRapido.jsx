...
import { SIN_DOBLE_FAZ, CON_ANILLADO, USA_ESCALA_BAJA } from './CalculadoraCotizaciones'; // IMPORTAMOS LA NUEVA REGLA
...
export const CotizadorRapido = () => {
    ...
    const calcularEscenario = (papel, esDobleFaz) => {
        ...
        // CORRECCIÓN MATEMÁTICA: Discriminamos la lógica según el papel para el cotizador
    ...
    if (parseInt(cantidadPaginas) > 0) {
        ...
        papelesDelGrupo.forEach((papel) => {
            ...
            if (datosSimple) {
                escenarios.push({
                    id: `${papel}-simple`, titulo: nombreMostrar, subtitulo: "Simple Faz",
                    datos: datosSimple, color: "bg-gray-800", textoColor: "text-gray-100", permiteAnillado: permiteAnillado
                });
            }
            ...
            if (permiteDobleFaz) {
                ...
                if (datosDoble) {
                    escenarios.push({
                        id: `${papel}-doble`, titulo: nombreMostrar, subtitulo: "Doble Faz",
                        datos: datosDoble, color: "bg-gradient-to-br from-empresa to-[#D12E9E]", textoColor: "text-white", permiteAnillado: permiteAnillado
                    });
                }
            }
        });
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 dark:border-gray-700 text-center">
                ...
                <div className="max-w-2xl mx-auto flex flex-col sm:flex-row gap-4">
                    <div className="flex-1 relative">
                        <select className="w-full h-20 px-6 bg-gray-50 dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-600 rounded-3xl font-bold text-lg text-center text-gray-800 dark:text-white outline-none focus:border-empresa cursor-pointer appearance-none transition-all" value={categoriaSeleccionada} onChange={(e) => setCategoriaSeleccionada(e.target.value)}>
                    ...
                    {escenarios.map((escenario) => (
                        <div key={escenario.id} className={`${escenario.color} rounded-3xl shadow-xl overflow-hidden flex flex-col transform transition-transform duration-300 hover:-translate-y-2`}>
                            <div className="p-5 text-center border-b border-white/10">
                                ...
                                <p className="text-[10px] opacity-60 mt-3 font-medium text-white bg-black/20 mx-auto w-max px-3 py-1 rounded-full">Unitario: ${escenario.datos?.costoUnitario}</p>
                            </div>
