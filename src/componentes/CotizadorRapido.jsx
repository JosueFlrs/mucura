...
export const CotizadorRapido = () => {
    ...
    if (parseInt(cantidadPaginas) > 0) {
        ...
        papelesDelGrupo.forEach((papel) => {
            ...
            const colorSimple = esBlancoYNegro
                ? "bg-gray-600 dark:bg-gray-700"
                : "bg-pink-500 dark:bg-pink-600";

            const colorDoble = esBlancoYNegro
                ? "bg-gradient-to-br from-gray-800 to-black dark:from-gray-900 dark:to-black"
                : "bg-gradient-to-br from-empresa to-[#80185e]";
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 dark:border-gray-700 text-center">
                ...
                <div className="max-w-2xl mx-auto flex flex-col sm:flex-row gap-4">
                    <div className="flex-1 relative">
                        <select className="w-full h-16 px-6 bg-gray-50 dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-600 rounded-3xl font-bold text-lg text-center text-gray-800 dark:text-white outline-none focus:border-empresa cursor-pointer appearance-none transition-all" value={categoriaSeleccionada} onChange={(e) => setCategoriaSeleccionada(e.target.value)}>
                    ...
                    <div className="w-full sm:w-48 relative">
                        <input type="number" className="w-full h-16 px-6 bg-gray-50 dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-600 rounded-3xl font-black text-4xl text-center text-gray-800 dark:text-white outline-none focus:border-empresa focus:ring-4 focus:ring-empresa/20 transition-all shadow-inner" value={cantidadPaginas} onChange={(e) => { const val = e.target.value; if (val === '') setCantidadPaginas(''); else setCantidadPaginas(Math.max(0, parseInt(val))); }} placeholder="0" autoFocus />
                    ...
                    {escenarios.map((escenario) => (
                        <div key={escenario.id} className={`${escenario.colorContenedor} text-white rounded-3xl shadow-xl overflow-hidden flex flex-col transform transition-transform duration-300 hover:-translate-y-2 relative`}>
                            ...
                            <div className="p-5 text-center border-b border-white/20 relative overflow-hidden min-h-[140px] flex flex-col justify-center">
                                ...
                                <p className="text-[10px] font-bold text-white bg-black/20 border border-white/10 mx-auto w-max px-3 py-1 rounded-full relative z-10 shadow-inner mt-4">
                                    Unitario: ${escenario.datos?.costoUnitario}
                                </p>
                            </div>

                            <div className="p-5 flex-grow bg-white dark:bg-gray-900 flex flex-col gap-4 text-gray-900 dark:text-gray-100">
