...
export const CotizadorRapido = () => {
    ...
    const calcularEscenario = (tipo, esDobleFaz) => {
        const cantidad = parseInt(cantidadPaginas) || 0;
        if (cantidad <= 0 || !tarifas) return null;

        const claveTarifa = tipo + (esDobleFaz ? 'DobleFaz' : '');
    ...
    const escenarios = [
        { id: 1, titulo: "A4 B/N", subtitulo: "Simple Faz", datos: calcularEscenario('a4BlancoYNegro', false), color: "bg-gray-800", textoColor: "text-gray-100" },
        { id: 2, titulo: "A4 B/N", subtitulo: "Doble Faz", datos: calcularEscenario('a4BlancoYNegro', true), color: "bg-gray-700", textoColor: "text-white" },
        { id: 3, titulo: "A4 Color", subtitulo: "Simple Faz", datos: calcularEscenario('a4Color', false), color: "bg-gradient-to-br from-blue-500 to-blue-700", textoColor: "text-white" },
        { id: 4, titulo: "A4 Color", subtitulo: "Doble Faz", datos: calcularEscenario('a4Color', true), color: "bg-gradient-to-br from-empresa to-[#D12E9E]", textoColor: "text-white" }
    ];
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 dark:border-gray-700 text-center">
                ...
                <div className="max-w-md mx-auto relative">
                    <input
                        ...
                        autoFocus
                    />
                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-empresa text-white px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-md">
                        Páginas
                    </div>
                </div>
            </div>
                    ...
                    {escenarios.map((escenario) => (
                        <div key={escenario.id} className={`${escenario.color} rounded-3xl shadow-xl overflow-hidden flex flex-col transform transition-transform duration-300 hover:-translate-y-2`}>
                            ...
                            <div className="p-6 text-center border-b border-white/10">
                                <h3 className={`text-2xl font-black ${escenario.textoColor} tracking-tight`}>{escenario.titulo}</h3>
